...
export function defineUserRulesFor(
  token: Jwt,
  ...
  switch (token.type) {
    case JwtType.API_TOKEN: {
      ...
      builder.can(AppAbilityAction.READ, "User");

      if (token.apiToken.type === ApiTokenType.SERVICE) {
        builder.can(AppAbilityAction.CREATE, "User");
        builder.can(AppAbilityAction.UPDATE, "User");
        builder.can(AppAbilityAction.DELETE, "User");
      }
